...
export default function Create(_props: PageProps) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('tipos-tramite.store'));
    };

    return (
        <AuthenticatedLayout
            header={
                <h2 className="text-xl font-semibold leading-tight text-gray-800">Crear Tipo de Trámite</h2>
            }
        >
            <Head title="Crear Tipo de Trámite" />
                    ...
                    <div className="bg-white p-6 shadow sm:rounded-lg">
                        <form onSubmit={submit} className="space-y-6">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <Link href={route('tipos-tramite.index')} className="text-sm text-gray-600 hover:text-gray-900">Cancelar</Link>
                            </div>
                        </form>
                    </div>
                </div>
