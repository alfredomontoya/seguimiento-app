...
function buildOptions(departamentos: DepartamentoType[], depth: number = 0): { value: string; label: string }[] {
    const result: { value: string; label: string }[] = [];
    for (const d of departamentos) {
        result.push({ value: d.id.toString(), label: `${'—'.repeat(depth)} ${d.nombre} (${d.sigla})`.trim() });
        result.push(...buildOptions(departamentos.filter(c => c.padre_id === d.id), depth + 1));
    }
    return result;
}

export default function Create({ departamentos }: PageProps<{ departamentos: DepartamentoType[] }>) {
    ...
    const padreOptions = buildOptions(departamentos);
    ...
    return (
        <AuthenticatedLayout
            header={
                <h2 className="text-xl font-semibold leading-tight text-gray-800">Crear Departamento</h2>
            }
        >
                    ...
                    <div className="bg-white p-6 shadow sm:rounded-lg">
                        <form onSubmit={submit} className="space-y-6">
                            ...
                            <div>
                                ...
                                <TextInput id="sigla" type="text" className="mt-1 block w-full" value={data.sigla} onChange={(e) => setData('sigla', e.target.value)} required />
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <Link href={route('departamentos.index')} className="text-sm text-gray-600 hover:text-gray-900">Cancelar</Link>
                            </div>
                        </form>
                    </div>
                </div>
