...
interface Derivacion {
    ...
    funcionario: { id: number; nombres: string; apellidos: string } | null;
...
interface TramiteDetail {
    ...
    persona: { id: number; nro_documento: string; nombres: string; apellidos: string; telefono: string | null; email: string | null; direccion: string | null } | null;
...
export default function Show({
    ...
    funcionarios: { id: number; nombres: string; apellidos: string; cargo: string }[];
}) {
    ...
    const reasignarForm = useForm({
        departamento_id: '',
        funcionario_id: '',
    });

    const entregarForm = useForm({
        glosa_entrega: '',
    });

    const handleDerivar = (e: React.FormEvent) => {
        e.preventDefault();
        derivarForm.post(route('tramites.derivar', tramite.id), {
            onSuccess: () => setShowDerivar(false),
        });
    };

    const handleReasignar = (e: React.FormEvent) => {
        e.preventDefault();
        reasignarForm.post(route('tramites.reasignar', tramite.id), {
            onSuccess: () => setShowReasignar(false),
        });
    };
    ...
    const handleEntregar = (e: React.FormEvent) => {
        e.preventDefault();
        entregarForm.post(route('tramites.entregar', tramite.id), {
            onSuccess: () => setShowEntregar(false),
        });
    };
                ...
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                        <div className="lg:col-span-2 space-y-6">
                                ...
                                <div className="p-6">
                                    <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                        <div>
                                            ...
                                            <dd className="mt-1 text-sm text-gray-900">{tramite.nro_secuencial}</dd>
                                        </div>
                                        <div>
                                            <dt className="text-sm font-medium text-gray-500">Fecha de Recepción</dt>
                                            <dd className="mt-1 text-sm text-gray-900">{tramite.fecha_recepcion}</dd>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <dd className="mt-1 text-sm text-gray-900">{tramite.tipo_tramite?.nombre ?? '-'}</dd>
                                        </div>
                                        <div>
                                        ...
                                        {tramite.fecha_termino && (
                                            <div>
                                                <dt className="text-sm font-medium text-gray-500">Fecha de Término</dt>
                                                <dd className="mt-1 text-sm text-gray-900">{tramite.fecha_termino}</dd>
                                            </div>
                                        )}
                                        {tramite.glosa_entrega && (
                                            <div className="sm:col-span-2">
                                                <dt className="text-sm font-medium text-gray-500">Glosa de Entrega</dt>
                                                <dd className="mt-1 text-sm text-gray-900">{tramite.glosa_entrega}</dd>
                                            </div>
                                        )}
                                    </dl>
                                </div>
                                    ...
                                    {tramite.persona ? (
                                        <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                            ...
                                            <div>
                                                ...
                                                <dd className="mt-1 text-sm text-gray-900">{tramite.persona.nro_documento}</dd>
                                            </div>
                                            {tramite.persona.telefono && (
                                                <div>
                                                    <dt className="text-sm font-medium text-gray-500">Teléfono</dt>
                                                    <dd className="mt-1 text-sm text-gray-900">{tramite.persona.telefono}</dd>
                                                </div>
                                            )}
                                            {tramite.persona.email && (
                                                <div>
                                                    <dt className="text-sm font-medium text-gray-500">Email</dt>
                                                    <dd className="mt-1 text-sm text-gray-900">{tramite.persona.email}</dd>
                                                </div>
                                            )}
                                            {tramite.persona.direccion && (
                                                <div className="sm:col-span-2">
                                                    <dt className="text-sm font-medium text-gray-500">Dirección</dt>
                                                    <dd className="mt-1 text-sm text-gray-900">{tramite.persona.direccion}</dd>
                                                </div>
                                            )}
                                        </dl>
                                    ) : (
                            ...
                            <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                                <div className="border-b border-gray-200 bg-gray-50 px-6 py-4">
                                    <h3 className="text-base font-semibold text-gray-900">Historial de Derivaciones</h3>
                                </div>
                                <div className="p-6">
                                            ...
                                            <table className="min-w-full divide-y divide-gray-200">
                                                <thead className="bg-gray-50">
                                                    <tr>
                                                        ...
                                                        <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Funcionario</th>
                                                        <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Glosa</th>
                                                        <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Derivado por</th>
                                                    </tr>
                                                </thead>
                                                <tbody className="divide-y divide-gray-200">
                                                    {tramite.derivaciones.map((d) => (
                                                        <tr key={d.id} className="hover:bg-gray-50">
                                                            ...
                                                            <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-500">
                                                                {d.funcionario ? `${d.funcionario.apellidos}, ${d.funcionario.nombres}` : '-'}
                                                            </td>
            ...
            <Modal show={showDerivar} onClose={() => setShowDerivar(false)} maxWidth="lg">
                <form onSubmit={handleDerivar} className="p-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <InputLabel htmlFor="derivar_funcionario_id" value="Funcionario" />
                            <ComboboxInput
                                options={[{ value: '', label: 'Sin funcionario' }, ...funcionarios.map((f) => ({ value: f.id.toString(), label: `${f.apellidos}, ${f.nombres} - ${f.cargo}` }))]}
                                value={derivarForm.data.funcionario_id}
                                onChange={(val) => derivarForm.setData('funcionario_id', val.toString())}
                                placeholder="Sin funcionario"
                            />
                            <InputError message={derivarForm.errors.funcionario_id} className="mt-2" />
                        </div>
                    ...
                    <div className="mt-6 flex justify-end gap-3">
                        ...
                        <PrimaryButton disabled={derivarForm.processing}>
                            Derivar
                        </PrimaryButton>
                    </div>
                </form>
            ...
            <Modal show={showReasignar} onClose={() => setShowReasignar(false)} maxWidth="lg">
                <form onSubmit={handleReasignar} className="p-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <InputLabel htmlFor="reasignar_funcionario_id" value="Funcionario" />
                            <ComboboxInput
                                options={[{ value: '', label: 'Sin funcionario' }, ...funcionarios.map((f) => ({ value: f.id.toString(), label: `${f.apellidos}, ${f.nombres} - ${f.cargo}` }))]}
                                value={reasignarForm.data.funcionario_id}
                                onChange={(val) => reasignarForm.setData('funcionario_id', val.toString())}
                                placeholder="Sin funcionario"
                            />
                            <InputError message={reasignarForm.errors.funcionario_id} className="mt-2" />
                        </div>
                    </div>
                    ...
                    <div className="mt-6 flex justify-end gap-3">
                        ...
                        <PrimaryButton disabled={reasignarForm.processing}>
                            Reasignar
                        </PrimaryButton>
                    </div>
                </form>
            ...
            <Modal show={showCancelar} onClose={() => setShowCancelar(false)} maxWidth="sm">
                <div className="p-6">
                    ...
                    <p className="mb-6 text-sm text-gray-600">
                        ¿Está seguro de que desea cancelar el trámite {tramite.nro_secuencial}? Esta acción no se puede deshacer.
                    </p>
                    <div className="flex justify-end gap-3">
                        <SecondaryButton onClick={() => setShowCancelar(false)}>
                            No, volver
                        </SecondaryButton>
                        <DangerButton onClick={handleCancelar}>
                            Sí, cancelar trámite
                        </DangerButton>
                    </div>
                </div>
            ...
            <Modal show={showEntregar} onClose={() => setShowEntregar(false)} maxWidth="lg">
                <form onSubmit={handleEntregar} className="p-6">
                    ...
                    <div className="mt-6 flex justify-end gap-3">
                        ...
                        <PrimaryButton disabled={entregarForm.processing}>
                            Entregar
                        </PrimaryButton>
                    </div>
                </form>
