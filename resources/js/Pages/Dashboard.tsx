...
interface DashboardProps extends PageProps {
    kpis: { total_mes: number; pendientes: number; completados: number; tiempo_promedio: number };
    ...
    ultimos_tramites: Array<{ id: number; nro_secuencial: string; referencia: string; estado: string }>;
    top_urgentes: Array<{ id: number; nro_secuencial: string; referencia: string; dias: number; estado: string }>;
...
function Badge({ estado }: { estado: string }) {
    return (
        <span
            className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${estadoColors[estado] ?? 'bg-gray-100 text-gray-800'}`}
        >
            {' '}
                ...
                <div className="w-full">
                    ...
                    <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div className="rounded-lg border-l-4 border-green-600 bg-white p-5 shadow-sm">
                            ...
                            <p className="mt-1 text-3xl font-bold text-green-700"> {kpis.total_mes} </p>{' '}
                        </div>{' '}
                        ...
                        <div className="rounded-lg border-l-4 border-emerald-500 bg-white p-5 shadow-sm">
                            ...
                            <p className="mt-1 text-3xl font-bold text-emerald-600"> {kpis.completados} </p>{' '}
                        </div>{' '}
                        <div className="rounded-lg border-l-4 border-blue-500 bg-white p-5 shadow-sm">
                            ...
                            <p className="mt-1 text-3xl font-bold text-blue-700">
                                ...
                                <span className="ml-1 text-base font-normal text-gray-500"> días </span>{' '}
                            </p>{' '}
                        </div>{' '}
                    </div>{' '}
                    {/* Charts Section */}{' '}
                    <div className="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
                        ...
                        <div className="rounded-lg bg-white p-5 shadow-sm">
                            ...
                            <div className="space-y-1.5">
                                {' '}
                                {por_dia.map((item) => {
                                    const pct = (item.total / maxTotal(por_dia)) * 100;
                                    return (
                                ...
                                {por_dia.length === 0 && <p className="text-sm text-gray-400"> Sin datos </p>}{' '}
                            </div>{' '}
                        </div>{' '}
                        {/* Por Semana */}{' '}
                        <div className="rounded-lg bg-white p-5 shadow-sm">
                            ...
                            <div className="space-y-1.5">
                                {' '}
                                {por_semana.map((item) => {
                                    const pct = (item.total / maxTotal(por_semana)) * 100;
                                    return (
                                ...
                                {por_semana.length === 0 && <p className="text-sm text-gray-400"> Sin datos </p>}{' '}
                            </div>{' '}
                        </div>{' '}
                        {/* Por Estado */}{' '}
                        <div className="rounded-lg bg-white p-5 shadow-sm">
                            ...
                            <div className="space-y-3">
                                {' '}
                                {por_estado.map((item) => {
                                    const total = por_estado.reduce((s, i) => s + i.total, 0);
                                ...
                                {por_estado.length === 0 && <p className="text-sm text-gray-400"> Sin datos </p>}{' '}
                            </div>{' '}
                        </div>{' '}
                    </div>{' '}
                    {/* Tables Section */}{' '}
                                ...
                                <table className="w-full text-left text-sm">
                                    ...
                                    <thead>
                                        ...
                                        <tr className="border-b border-gray-200 text-xs uppercase text-gray-500">
                                            ...
                                            <th className="pb-2 pr-2 font-medium"> Referencia </th>{' '}
                                    ...
                                    <tbody>
                                        ...
                                        {top_urgentes.map((t) => (
                                            <tr key={t.id} className="border-b border-gray-100">
                                                ...
                                                <td className="py-2 pr-2 font-medium text-red-600"> {t.dias} </td>{' '}
                                                <td className="py-2">
                                                    {' '}
                                                    <Badge estado={t.estado} />{' '}
                                                </td>{' '}
                        ...
                        <div className="rounded-lg bg-white p-5 shadow-sm">
                            ...
                            <div className="space-y-2">
                                ...
                                {carga_departamentos.map((dept) => {
                                    const maxCarga = Math.max(1, ...carga_departamentos.map((d) => d.tramites_count));
                                    const pct = (dept.tramites_count / maxCarga) * 100;
                                    return (
                                        <div key={dept.nombre}>
                                            {' '}
                                            <div className="mb-1 flex items-center justify-between text-sm">
                                                {' '}
                                                <span className="text-gray-700"> {dept.nombre} </span>{' '}
