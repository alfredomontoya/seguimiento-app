...
export default function Edit({ funcionario, users, departamentos }: PageProps<{ funcionario: Funcionario; users: User[]; departamentos: Departamento[] }>) {
    const { data, setData, put, processing, errors } = useForm({
        user_id: funcionario.user_id?.toString() ?? '',
        nombres: funcionario.nombres,
        apellidos: funcionario.apellidos,
        cargo: funcionario.cargo ?? '',
        profesion: funcionario.profesion ?? '',
        departamento_id: funcionario.departamento_id.toString(),
    });
    ...
    return (
        <AuthenticatedLayout
            header={
                <h2 className="text-xl font-semibold leading-tight text-gray-800">Editar Funcionario</h2>
            }
        >
