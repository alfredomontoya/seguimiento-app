...
export default function Edit({
    ...
    permisos,
    permisoIds,
}: PageProps<{ rol: Rol; permisos: Permiso[]; permisoIds: number[] }>) {
    ...
    const {
        data: permisoData,
        setData: setPermisoData,
        post: syncPermisos,
        processing: syncing,
    } = useForm({ permiso_ids: permisoIds });
    ...
    const togglePermiso = (permisoId: number) => {
        ...
        if (current.includes(permisoId)) {
            setPermisoData(
                'permiso_ids',
                current.filter((id: number) => id !== permisoId),
            );
        } else {
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Editar Rol</h2>}>
            {' '}
                                ...
                                {permisos.map((p) => (
                                    ...
                                    >
                                        ...
                                        <input
                                            ...
                                            checked={permisoData.permiso_ids.includes(p.id)}
